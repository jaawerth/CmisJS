...
(function(root, factory) {
  ...
  lib.createSession = function(url) {
     ...
     * @return {CmisRequest}
     */
    session.createDocument = function(parentId, content, input, mimeTypeExtension, versioningState, policies, addACEs, removeACEs, options) {
      var options = _fill(options);
      if ('string' == typeof input) {
        input = {
          'cmis:name': input
      ...
      return _postMultipart(session.defaultRepository.rootFolderUrl,
        options, content, mimeTypeExtension, properties['cmis:name']);
     ...
     * @return {CmisRequest}
     */
    session.createDocumentFromSource = function(parentId, sourceId, content, input, mimeTypeExtension, versioningState, policies, addACEs, removeACEs, options) {
      var options = _fill(options);
      if ('string' == typeof input) {
        input = {
          'cmis:name': input
    ...
    function CmisRequest(req, text) {
      ...
      this.then = function(onResolve, onReject) {
        return cmisRequestToPromise(this).then(onResolve, onReject);
      };


      this.catch = function(onReject) {
        return cmisRequestToPromise(this).catch(onReject);
      };

      function cmisRequestToPromise(cmr) {
    ...
    var _postMultipart = function(url, options, content, mimeTypeExtension, filename) {
      ...
      if (content) {
        ...
        req.attach("content", content, mimeTypeExtension ? filename + '.' + mimeTypeExtension : filename);
      }
      for (var k in options) {
